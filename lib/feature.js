...
export default class Feature extends AxosoftDomElement {
    ...
    onClick(event) {
        ...
        for (let i = 0; i < event.path.length; ++i) {
            ...
            if (this.elements.childrenList) {
                //  Clicking on a child item should not trigger
                //  any action on the parent.
                if (event.path[i] == this.elements.childrenList) {
                    return;
                }
            }
        ...
        if ( ! isOpen) {
            console.log('Opening ' + this.feature.id);
